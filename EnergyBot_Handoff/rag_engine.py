    ...
    def _try_dataframe_answer(self, question: str) -> Optional[Dict[str, Any]]:
        ...
        if age_match:
            try:
                ...
                if not matches.empty:
                    answer_header = f"📊 **Analyse: {len(matches)} Objekte > {threshold} Jahre**"
                    exact_count_msg = f"Es wurden insgesamt **{len(matches)} Hausanschlüsse** gefunden, die älter als {threshold} Jahre sind."
        ...
        if any(re.search(rf'\b{x}\b', ql) for x in ["risiko", "risk", "kritisch", "kritisches", "critical", "hoch", "high", "low", "niedrig", "gut", "mittel", "medium", "mittleres"]):
            try:
                if any(re.search(rf'\b{x}\b', ql) for x in ["kein", "no", "niedrig", "low", "unbedenklich", "gut"]):
                    matches = self.unified_df[self.unified_df["Risiko"] == "Niedrig"]
                    status_text = "mit geringem oder keinem Risiko"
                    emoji = "✅"
                elif any(re.search(rf'\b{x}\b', ql) for x in ["mittel", "medium", "mittleres"]):
                    matches = self.unified_df[self.unified_df["Risiko"] == "Mittel"]
                    status_text = "mit mittlerem Risiko"
                    emoji = "⚠️"
                elif any(re.search(rf'\b{x}\b', ql) for x in ["hoch", "high", "kritisch", "kritisches", "critical"]):
                    matches = self.unified_df[self.unified_df["Risiko"] == "Hoch"]
                    status_text = "hochrisikig / kritisch"
                    emoji = "🚨"
                else:
                    matches = self.unified_df[self.unified_df["Risiko"].isin(["Hoch", "Mittel"])]
                    status_text = "mit erhöhtem Risiko"
                    emoji = "🚨"
                    
                if not matches.empty:
                    answer_header = f"{emoji} **Risiko-Analyse: {len(matches)} Objekte gefunden**"
                    exact_count_msg = f"Es wurden insgesamt **{len(matches)} Hausanschlüsse** als **{status_text}** eingestuft."
        ...
        try:
            ...
            if is_update:
                ...
                payload = {
                    ...
                    "messages": [
                        {"role": "system", "content": (
                            "You are an infrastructure data expert for STADTWERKE X. "
                            "Answer questions accurately based on the provided Data. "
                            "RISK CATEGORIES:\n"
                            "- 'Gut' = Niedriges Risiko (Low/No Risk)\n"
                            "- 'Mittel' = Mittleres Risiko (Medium Risk)\n"
                            "- 'Risiko' = Hohes Risiko (High Risk / 'Hoch' category)\n\n"
                            "RISK RULES (Age in Years = 2026 - Install Year):\n"
                            "- GAS:\n"
                            "  * Stahl mit KKS: gut(0-59), mittel(60-95), Risiko(>=96)\n"
                            "  * Stahl ohne KKS: gut(0-51), mittel(52-83), Risiko(>=84)\n"
                            "  * PE: gut(0-55), mittel(56-89), Risiko(>=90)\n"
                            "- WASSER:\n"
                            "  * AZ: gut(0-36), mittel(37-59), Risiko(>=60)\n"
                            "  * PE: gut(0-62), mittel(63-101), Risiko(>=102)\n"
                            "  * PVC: gut(0-36), mittel(37-59), Risiko(>=60)\n"
                            "  * Stahl: gut(0-44), mittel(45-71), Risiko(>=72)\n\n"
                            "MODERNIZATION KPI (Infrastruktur Modernisierung nötig):\n"
                            "A connection needs modernization if its Age > 'Technische Nutzungsdauer'.\n"
                            "Keep it concise, friendly, and in the German language by default unless asked otherwise."
                        )}
                    ],
                    "temperature": 0.1,
                    "max_tokens": 600
