...
2.  **Prepare Maps**:
    *   Run `setup_osrm.bat`. This downloads and processes regional map data into the `osrm_data/` folder.
3.  **Launch**:
    ```bash
    docker compose up --build -d
    ```
4.  **Access**:
    *   **App UI**: `http://localhost:8600`
    *   **OSRM API**: `http://localhost:5000` (Backend routing)
